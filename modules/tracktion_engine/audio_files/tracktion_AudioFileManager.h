...
public:
    ...
    /// BSV-2473 step 2: create a fixed-shape alias slot. The data pointer starts
    /// null (silence) until repointAliasSlot() is called. Message thread only.
    void createAliasSlot (const std::string& name, int numFrames, int numChannels, double sampleRate);

    /// BSV-2473 step 2: atomically re-point an existing alias slot's data
    /// pointer. Pure mechanism — the caller resolves/validates the source and
    /// computes the pointer; this call is pointer-arithmetic-agnostic. Returns
